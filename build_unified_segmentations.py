...
import numpy as np
MAPPING = {'output': {'6': {'source_labels': [6], 'centerline_mode': 'single'}, '9': {'source_labels': [9, 11], 'centerline_mode': 'multi'}, '10': {'source_labels': [10, 12], 'centerline_mode': 'multi'}}}
...
def main():
    ...
    p.add_argument('--input-dir', type=Path, default=Path(__file__).resolve().parent / 'segments_labelled_v1')
    p.add_argument('--output-dir', type=Path, default=Path(__file__).resolve().parent / 'segments_labelled_v1_unified')
    args = p.parse_args()
