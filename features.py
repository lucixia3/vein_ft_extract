...
LABEL_DICT: Dict[int, str] = {1: 'VOG', 2: 'STS', 3: 'ICV', 4: 'RBVR', 6: 'SSS', 7: 'TransvSig-R', 8: 'TransvSig-L', 9: 'Cortical-R', 10: 'Cortical-L', 11: 'VCM-R', 12: 'VCM-L'}
ICV_STRUCTURE_FILES: List[str] = ['brainstem.nii.gz', 'venous_sinuses.nii.gz', 'caudate_nucleus.nii.gz', 'cerebellum.nii.gz', 'subarachnoid_space.nii.gz', 'septum_pellucidum.nii.gz', 'lentiform_nucleus.nii.gz', 'insular_cortex.nii.gz', 'internal_capsule.nii.gz', 'ventricle.nii.gz', 'central_sulcus.nii.gz', 'frontal_lobe.nii.gz', 'parietal_lobe.nii.gz', 'occipital_lobe.nii.gz', 'temporal_lobe.nii.gz', 'thalamus.nii.gz']
TOTALSEG_COMPLETENESS_MARKERS: List[str] = ['brainstem.nii.gz', 'venous_sinuses.nii.gz', 'subarachnoid_space.nii.gz', 'ventricle.nii.gz', 'thalamus.nii.gz']

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=__doc__)
    parser.add_argument('--project-root', type=Path, default=Path(__file__).resolve().parents[1], help='Root directory of the project. Default: parent of the scripts folder.')
    parser.add_argument('--cta-dir', type=Path, default=None, help='CTA directory. Default: <project-root>/cases/cta')
    parser.add_argument('--seg-dir', type=Path, default=None, help='Vein segmentation directory. Default: <project-root>/cases/segments')
    parser.add_argument('--results-dir', type=Path, default=None, help='Results directory. Default: <project-root>/results')
    parser.add_argument('--case-id', type=str, nargs='+', default=None, help='One or more case IDs to process, for example: --case-id sub-stroke_0002 sub-stroke_0004. If omitted, all matched CTA/segmentation pairs are processed.')
    parser.add_argument('--force-totalseg', action='store_true', help='Re-run TotalSegmentator even if an apparently complete output folder already exists. This mainly affects the regenerated TotalSegmentator masks, the saved ICV mask, and downstream ICV-based quantities such as volume_fraction_icv_percent.')
...
def get_case_pairs(cta_dir: Path, seg_dir: Path, selected_case_ids: set[str] | None=None) -> List[Tuple[str, Path, Path]]:
...
def extract_case_features(case_id: str, cta_path: Path, seg_path: Path, ts_dir: Path, case_results_dir: Path) -> pd.DataFrame:
    ...
    nib.save(nib.Nifti1Image(icv_mask.astype(np.uint8), cta_nii.affine, cta_nii.header), str(icv_mask_path))
    rows = []
    for label in sorted((int(v) for v in np.unique(seg) if v != 0)):
        ...
        rows.append({'case': case_id, 'label': label, 'structure': LABEL_DICT.get(label, 'Unknown'), 'volume_mm3': volume_mm3, 'total_vein_volume_mm3': total_vein_volume_mm3, 'icv_volume_mm3': icv_volume_mm3, 'n_connected_components': n_components, 'mean_hu_abs': float(np.mean(values)) if values.size > 0 else float('nan'), 'std_hu_abs': float(np.std(values)) if values.size > 0 else float('nan'), 'min_hu_abs': min_hu, 'max_hu_abs': max_hu, 'p10_hu_abs': p10_hu, 'p90_hu_abs': p90_hu, 'max_diameter_mm': dia['max_diameter_mm'], 'min_diameter_mm': dia['min_diameter_mm'], 'mean_diameter_mm': dia['mean_diameter_mm'], 'median_diameter_mm': dia['median_diameter_mm'], 'std_diameter_mm': dia['std_diameter_mm'], 'p10_diameter_mm': dia['p10_diameter_mm'], 'p90_diameter_mm': dia['p90_diameter_mm'], 'n_skeleton_voxels': dia['n_skeleton_voxels']})
    ...
    return df[['case', 'label', 'structure', 'volume_mm3', 'total_vein_volume_mm3', 'icv_volume_mm3', 'n_connected_components', 'mean_hu_abs', 'std_hu_abs', 'min_hu_abs', 'max_hu_abs', 'p10_hu_abs', 'p90_hu_abs', 'mean_hu_ref_sss', 'std_hu_ref_sss', 'volume_fraction_all_veins_percent', 'volume_fraction_icv_percent', 'max_diameter_mm', 'min_diameter_mm', 'mean_diameter_mm', 'median_diameter_mm', 'std_diameter_mm', 'p10_diameter_mm', 'p90_diameter_mm', 'n_skeleton_voxels']]
...
def main() -> int:
    ...
    for case_id, cta_path, seg_path in pairs:
        ...
        try:
            ...
            failure_rows.append({'case': case_id, 'cta_file': str(cta_path), 'seg_file': str(seg_path), 'error_type': exc.__class__.__name__, 'reason': short_reason, 'details': error_text})
