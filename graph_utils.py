...
def filter_nodes_incident_to_edges(nodes_df: pd.DataFrame, edges_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def compute_inter_component_bridges(parts: list[dict], label: int, max_bridge_mm: float=45.0) -> pd.DataFrame:
    n = len(parts)
    if n < 2:
        return pd.DataFrame()
    ...
    for mult in (1.0, 1.5, 2.25):
        ...
        for dist, i, j, ni, nj, pi, pj in pairs:
            ...
            if find(i) == find(j):
                continue
            union(i, j)
            bridge_rows.append({'label': label, 'comp_a': int(parts[i]['comp_id']), 'node_a': int(ni), 'comp_b': int(parts[j]['comp_id']), 'node_b': int(nj), 'dist_mm': round(float(dist), 4), 'xa': float(pi[0]), 'ya': float(pi[1]), 'za': float(pi[2]), 'xb': float(pj[0]), 'yb': float(pj[1]), 'zb': float(pj[2]), 'is_bridge': 1})
        if len({find(k) for k in range(n)}) == 1:
            break
    return pd.DataFrame(bridge_rows)

def save_bridges_for_label_dir(label_dir: Path, label: int, max_bridge_mm: float=45.0) -> None:
