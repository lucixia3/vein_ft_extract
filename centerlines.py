...
def prune_skeleton(skel, min_branch_voxels=20):
    ...
    kernel[1, 1, 1] = 0
    offsets = [(di, dj, dk) for di in [-1, 0, 1] for dj in [-1, 0, 1] for dk in [-1, 0, 1] if not (di == 0 and dj == 0 and (dk == 0))]
    skel = skel.copy()
    shape = skel.shape
    for _ in range(100):
        ...
        for ep in endpoints:
            ...
            while True:
                ...
                ci, cj, ck = current
                nbs = [(ci + di, cj + dj, ck + dk) for di, dj, dk in offsets if 0 <= ci + di < shape[0] and 0 <= cj + dj < shape[1] and (0 <= ck + dk < shape[2]) and skel[ci + di, cj + dj, ck + dk] and ((ci + di, cj + dj, ck + dk) != previous)]
                if not nbs or len(nbs) >= 2 or len(branch) >= min_branch_voxels:
                    break
                previous, current = (current, nbs[0])
            if len(branch) < min_branch_voxels:
                li, lj, lk = branch[-1]
                deg = sum((1 for di, dj, dk in offsets if 0 <= li + di < shape[0] and 0 <= lj + dj < shape[1] and (0 <= lk + dk < shape[2]) and skel[li + di, lj + dj, lk + dk]))
                if deg < 3:
                    for v in branch:
                        skel[v] = False
                    removed_any = True
...
def cta_matches_segmentation(cta_nii, seg_shape: tuple, seg_affine: np.ndarray) -> bool:
    ...
    if sh != tuple((int(x) for x in seg_shape[:3])):
        return False
    return bool(np.allclose(cta_nii.affine, seg_affine, rtol=1e-05, atol=0.0001))
...
def hu_sampled_along_branches(cta_data: np.ndarray, affine: np.ndarray, branches):
    ...
    for pts in branches:
        ...
        vals = map_coordinates(np.asarray(cta_data, dtype=float), ijk, order=1, mode='nearest')
...
def smooth_centerline(pts, sigma=2.0):
    ...
    if len(pts) < 5:
        return pts
    return np.column_stack([gaussian_filter1d(pts[:, 0], sigma=sigma), gaussian_filter1d(pts[:, 1], sigma=sigma), gaussian_filter1d(pts[:, 2], sigma=sigma)])
...
def skeleton_to_branches_multi_with_graph(skel, affine, spacing, min_branch_mm=5.0, smooth_sigma=2.0):
    ...
    for gid in special_sorted:
        ...
        nodes_rows.append({'node_id': gid_to_nid[gid], 'graph_vertex_id': int(gid), 'degree': deg, 'role': role, 'x': float(xyz[0]), 'y': float(xyz[1]), 'z': float(xyz[2])})
    ...
    for node in special:
        for nb in adj[node]:
            ...
            if length >= min_branch_mm:
                ...
                branches.append(pts)
                branch_rows.append({'branch_id': bid, 'node_start': gid_to_nid[node], 'node_end': gid_to_nid[end_node], 'length_mm': round(length, 4)})
                bid += 1
...
def skeleton_to_branches_multi(skel, affine, spacing, min_branch_mm=5.0):
    branches, _, _ = skeleton_to_branches_multi_with_graph(skel, affine, spacing, min_branch_mm=min_branch_mm)
    return branches
...
def single_branch_with_graph(voxels, adj, spacing, affine, smooth_sigma=2.0):
    ...
    nodes_df = pd.DataFrame([{'node_id': 0, 'graph_vertex_id': n0, 'degree': d0, 'role': 'endpoint', 'x': float(p0[0]), 'y': float(p0[1]), 'z': float(p0[2])}, {'node_id': 1, 'graph_vertex_id': n1, 'degree': d1, 'role': 'endpoint', 'x': float(p1[0]), 'y': float(p1[1]), 'z': float(p1[2])}])
    length_mm = float(np.linalg.norm(np.diff(pts_mm, axis=0), axis=1).sum())
    edges_df = pd.DataFrame([{'branch_id': 0, 'node_start': 0, 'node_end': 1, 'length_mm': round(length_mm, 4)}])
...
def compute_torsion(pts):
    ...
    for i in range(1, len(pts) - 2):
        ...
        if nb1 < 1e-09 or nb2 < 1e-09:
            continue
        ds = np.linalg.norm(t2)
        out[i] = np.arccos(np.clip(np.dot(b1 / nb1, b2 / nb2), -1, 1)) / ds if ds > 0 else np.nan
    return out

def branches_to_dataframe(branches, label, comp_id, diameter_per_branch=None, hu_per_branch=None):
    rows = []
    for bid, pts in enumerate(branches):
        ...
        for i, (x, y, z) in enumerate(pts):
            row = {'label': label, 'component': comp_id, 'branch_id': bid, 'point_id': i, 'x': x, 'y': y, 'z': z, 'curvature': float(curv[i]) if not np.isnan(curv[i]) else np.nan, 'torsion': float(tors[i]) if not np.isnan(tors[i]) else np.nan}
            if diam is not None:
                row['diameter_mm'] = float(diam[i]) if not np.isnan(diam[i]) else np.nan
...
def compute_branch_stats(df):
    ...
    for (label, comp, bid), grp in df.groupby(['label', 'component', 'branch_id']):
        ...
        rec = {'label': label, 'component': comp, 'branch_id': bid, 'n_points': len(grp), 'length_mm': round(length_mm, 3), 'euclidean_mm': round(euclidean, 3), 'tortuosity': round(tortuosity, 4) if not np.isnan(tortuosity) else np.nan, 'curvature_mean': round(grp['curvature'].mean(), 5), 'curvature_max': round(grp['curvature'].max(), 5), 'torsion_mean': round(grp['torsion'].mean(), 5)}
        if 'diameter_mm' in grp.columns and grp['diameter_mm'].notna().any():
            ...
            rec['diameter_std_mm'] = round(float(g.std()), 4) if len(g) > 1 else 0.0
...
def run_pipeline(input_path, output_dir, target_labels=None, mode='single', label_modes=None, prune_voxels=20, smooth_sigma=2.0, min_branch_mm=5.0, min_voxels=50, save_skeleton=False, bridge_max_mm=45.0, bridge_labels=(9, 10), cta_path=None):
    out = Path(output_dir)
    ...
    if cta_path:
        ...
        if cta_matches_segmentation(cn, data.shape, affine):
            ...
            log.warning(f'CTA no coincide en shape/affine con la segmentación; se omite HU. cta={cn.shape[:3]} seg={data.shape}')
    ...
    for label in labels_to_process:
        ...
        for comp_id, comp_mask, n_voxels in components:
            ...
            if eff_mode == 'single':
                voxels, adj = build_graph(skel)
                branches, nodes_df, edges_df = single_branch_with_graph(voxels, adj, spacing, affine, smooth_sigma)
            else:
                branches, nodes_df, edges_df = skeleton_to_branches_multi_with_graph(skel, affine, spacing, min_branch_mm, smooth_sigma)
            if not branches:
                log.warning('    Sin ramas validas.')
                continue
            if not nodes_df.empty and (not edges_df.empty):
                nodes_df, edges_df = filter_nodes_incident_to_edges(nodes_df, edges_df)
            ...
            if not edges_df.empty:
                ...
                all_edges.append(ed)
            diam_list = diameter_sampled_along_branches_mm(branches, comp_mask.astype(bool), affine, spacing)
            hu_list = hu_sampled_along_branches(cta_arr, affine, branches) if cta_arr is not None else None
            df_pts = branches_to_dataframe(branches, label, comp_id, diameter_per_branch=diam_list, hu_per_branch=hu_list)
    ...
    if all_nodes:
        save_csv(pd.concat(all_nodes, ignore_index=True), str(out / 'all_graph_nodes.csv'))
    if all_edges:
        save_csv(pd.concat(all_edges, ignore_index=True), str(out / 'all_graph_edges.csv'))
...
def parse_args():
    ...
    p.add_argument('--mode', choices=['single', 'multi'], default='single', help='single=camino principal | multi=todas las ramas (default: single)')
    p.add_argument('--prune-voxels', type=int, default=20, help='Eliminar ramas del esqueleto con menos de N voxels (default: 20)')
    p.add_argument('--smooth-sigma', type=float, default=2.0, help='Suavizado gaussiano del centerline (default: 2.0)')
    p.add_argument('--min-branch-mm', type=float, default=5.0, help='Longitud minima de rama en modo multi, mm (default: 5.0)')
    p.add_argument('--min-voxels', type=int, default=50)
    p.add_argument('--save-skeleton', action='store_true')
    p.add_argument('--cta', type=str, default=None, help='NIfTI CTA (misma shape/affine que la segmentación) para columna hu en centerlines.')
    return p.parse_args()
if __name__ == '__main__':
    args = parse_args()
    run_pipeline(input_path=args.input, output_dir=args.output, target_labels=args.labels, mode=args.mode, prune_voxels=args.prune_voxels, smooth_sigma=args.smooth_sigma, min_branch_mm=args.min_branch_mm, min_voxels=args.min_voxels, save_skeleton=args.save_skeleton, cta_path=args.cta)
