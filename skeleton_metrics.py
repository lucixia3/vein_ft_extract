...
def skeleton_extended_diameter_stats_mm(mask: np.ndarray, spacing: Tuple[float, float, float]) -> Dict[str, Any]:
    out: Dict[str, Any] = {'max_diameter_mm': float('nan'), 'min_diameter_mm': float('nan'), 'mean_diameter_mm': float('nan'), 'std_diameter_mm': float('nan'), 'median_diameter_mm': float('nan'), 'p10_diameter_mm': float('nan'), 'p90_diameter_mm': float('nan'), 'n_skeleton_voxels': 0}
    if np.count_nonzero(mask) == 0:
        return out
    distance_map = ndi.distance_transform_edt(mask, sampling=spacing)
    skeleton = skeletonize(mask.astype(bool))
