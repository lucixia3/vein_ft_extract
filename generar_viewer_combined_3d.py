    ...
    for label, (color, _desc) in LABEL_PALETTE.items():
        ...
        for csv_p in comp_csvs:
            ...
            if np_nodes.exists() and np_edges.exists():
                ...
                nd = nd.drop(
                    columns=[c for c in ("label", "component") if c in nd.columns],
                    errors="ignore",
                )
                ed = ed.drop(
                    columns=[c for c in ("label", "component") if c in ed.columns],
                    errors="ignore",
                )
                ...
                if et is not None:
                    ...
                    et["name"] = f"[{LABEL_PALETTE[label][1]}] comp{comp_id} aristas"
                    et["legendgroup"] = "edges"
                    traces.append(et)
                nt = trace_graph_nodes(nd, comp_id)
                if nt is not None:
                    nt["name"] = f"[{LABEL_PALETTE[label][1]}] comp{comp_id} nodos"
                ...
                traces.append(
                    {
                        ...
                        "z": zs,
                        "mode": "lines",
                        "name": f"[{LABEL_PALETTE[label][1]}] puentes",
                        "line": {
    ...
    for label in (6, 9, 10):
        ...
        for nf in sorted(ld.glob("comp_*_graph_nodes.csv")):
            ...
            for _, r in nd.iterrows():
                ...
                deg = (
                    int(r["degree"])
                    if "degree" in nd.columns and pd.notna(r.get("degree"))
                    else 0
                )
                G.add_node(
                    k,
...
def build_ortho_graph_traces(G: nx.Graph, plane: str) -> list[dict]:
    ...
    seg_edges: dict[int, tuple[list[float], list[float]]] = {
        6: ([], []),
        9: ([], []),
        10: ([], []),
    }
    ...
    out: list[dict] = []
    edge_style = [
        (6, "L6 — tronco (recto)", LABEL_PALETTE[6][0], 6),
        (9, "L9 — ramificaciones", LABEL_PALETTE[9][0], 3.5),
        (10, "L10 — ramificaciones", LABEL_PALETTE[10][0], 3.5),
    ]
        ...
        out.append(
            {
                ...
                "y": by,
                "mode": "lines",
                "name": "Puentes (proyectados)",
                "line": {"color": "rgba(140,80,200,0.95)", "width": 3, "dash": "dash"},
                "hoverinfo": "skip",
                "showlegend": True,
            }
        )

    for lab, leg_name, color in (
        (6, "Nodos L6 (extremos tronco)", LABEL_PALETTE[6][0]),
        (9, "Nodos L9 (bifurcaciones / extremos)", LABEL_PALETTE[9][0]),
        (10, "Nodos L10 (bifurcaciones / extremos)", LABEL_PALETTE[10][0]),
    ):
        ...
        texts = [
            f"Etiq {lab} · comp {k.split(':')[1]} · nodo {k.split(':')[2]}"
            + (
                " · bifurcación"
                if (
                    str(G.nodes[k].get("role", "")).lower() == "junction"
                    or int(G.nodes[k].get("degree") or 0) >= 3
                )
                else " · extremo"
            )
            for k in subk
...
def write_combined_html(
    ...
    ortho_specs = [
        ("yz", "Y (mm)", "Z (mm)", "Plano YZ (vista desde +X) — L6 vs L9 / L10"),
        ("xz", "X (mm)", "Z (mm)", "Plano XZ (vista desde +Y) — L6 vs L9 / L10"),
        ("xy", "X (mm)", "Y (mm)", "Plano XY (vista desde +Z) — L6 vs L9 / L10"),
    ]
    ...
    for pkey, xt, yt, tit in ortho_specs:
        ...
        if tr:
            any_topo = True
        ortho_payload.append(
            {
                "div_id": f"plot_topo_{pkey}",
                "traces": tr,
                "layout": _figure_layout_ortho(xt, yt, tit),
            }
        )
    n_n = int(G.number_of_nodes()) if G is not None else 0
    n_e = int(G.number_of_edges()) if G is not None else 0
    topo_sub = (
        f"Tres proyecciones ortográficas (YZ, XZ, XY) en mm; azul grueso = L6; verde/naranja = L9/L10. "
        f"{n_n} nodos, {n_e} aristas."
        if any_topo
        else "Sin datos de grafo (faltan x,y,z en graph_nodes o CSV de aristas / resultados)."
    )
    data_json = json.dumps({"traces": traces, "ortho": ortho_payload})
    topo_sub_esc = (
        topo_sub.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    )
...
.graph2d-sub {{ font-size:10px; color:#6b7280; padding:0 16px 8px; line-height:1.45; }}
...
<header>
  ...
  <div class="toggle-wrap">
    ...
    <label><input type="checkbox" id="tg-cl" checked onchange="upd()"> Centerlines</label>
    <label><input type="checkbox" id="tg-edges" checked onchange="upd()"> Aristas</label>
    <label><input type="checkbox" id="tg-bridges" checked onchange="upd()"> Puentes</label>
    <label><input type="checkbox" id="tg-nodes" checked onchange="upd()"> Nodos</label>
  </div>
</header>
<div id="plot3d"></div>
<div class="graph2d-sub" style="border-top:1px solid #2a2d3e;padding-top:8px;">{topo_sub_esc}</div>
<details class="topo-details" open>
<summary>Plano YZ (desde +X)</summary>
<div id="plot_topo_yz" class="plot-ortho"></div>
</details>
<details class="topo-details" open>
<summary>Plano XZ (desde +Y)</summary>
<div id="plot_topo_xz" class="plot-ortho"></div>
</details>
<details class="topo-details" open>
<summary>Plano XY (desde +Z)</summary>
...
def main():
    ...
    p.add_argument(
        ...
        required=True,
        type=Path,
        help="Carpeta del caso (ej. centerlines_unified_v1/CTA_final_hybrid_002)",
    )
    p.add_argument(
        "--seg",
        required=True,
        help="NIfTI unificado alineado (segments_labelled_v1_unified/...)",
    )
    ...
    write_combined_html(
        traces, out, "Visor 3D + vista 2D desplegable — L6, L9, L10", args.resultados, args.seg
    )
    print(f"Listo: {out}")
